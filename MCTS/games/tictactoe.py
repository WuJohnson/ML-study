...
class TicTacToeGameState(TwoPlayersGameState):
	"""
	3*3 cells: default cell value = 0; x = 1, o = -1
	state: game state = board(blank) + current moves
	next_to_move: o(-1) or x(1)
	"""
	x = 1
	o = -1
	...
	def game_result(self):
		...
		diag_sum_tl = self.board.trace()        # -对角线
		diag_sum_tr = self.board[::-1].trace()

		# x win
		if any(rowsum == self.board_size) or \
				any(colsum == self.board_size) or \
				diag_sum_tl == self.board_size or \
				diag_sum_tr == self.board_size:
			return 1.
		# o win
		elif any(rowsum == -self.board_size) or \
				any(colsum == -self.board_size) or \
				diag_sum_tl == -self.board_size or \
				diag_sum_tr == -self.board_size:
			return -1.
